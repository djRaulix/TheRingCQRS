namespace TheRing.CQRS.RavenDb
{
        ...
        where TSaga : class, ISaga
    {
        ...
        public SagaRepository(IDocumentStoreFactory documentStoreFactory)
        {
            this.documentStore = documentStoreFactory.SagaStore;
        }
        ...
        public IEnumerable<Action<IConsumeContext<TMessage>>> GetSaga<TMessage>(
            IConsumeContext<TMessage> context, Guid sagaId, InstanceHandlerSelector<TSaga, TMessage> selector, ISagaPolicy<TSaga, TMessage> policy) where TMessage : class
        {
        ...
        public IEnumerable<TResult> Select<TResult>(Func<TSaga, TResult> transformer)
        {
            using (var session = this.documentStore.OpenSession())
            {
                var result = session.Query<TSaga>().Customize(c => c.WaitForNonStaleResults()).Select(transformer).ToList();
        ...
        public IEnumerable<TSaga> Where(ISagaFilter<TSaga> filter)
        {
            using (var session = this.documentStore.OpenSession())
            {
                var result = session.Query<TSaga>().Where(filter.FilterExpression).Customize(c => c.WaitForNonStaleResults()).ToList();
